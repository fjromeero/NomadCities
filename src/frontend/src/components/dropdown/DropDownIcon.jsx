...
export default function DropDownIcon(props) {
    ...
    return (
        <li ref={menuRef} className="w-20 flex align-middle justify-center">
            <button
                className={screen.width<830 && open ? "w-8 h-8 bg-black rounded-[50%] p-[5px] flex align-middle justify-center" : "w-8 h-8 bg-[#979797] rounded-[50%] p-[5px] flex align-middle justify-center"} 
                onClick={() => setOpen(!open)}
            >
                    {
                        screen.width<830 && open 
